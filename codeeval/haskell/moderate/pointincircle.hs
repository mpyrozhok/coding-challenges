...
main = getContents >>= putStr . unlines . process . lines
    where
        process = map (boolToStr . elements . map (head . drop 1 . splitOn ":") . splitOn ";" . filter (`notElem` "()"))
        strToFloats :: String -> Point
        strToFloats xs = let [x, y] = map read . splitOn "," $ xs in Point x y
        elements xs = let p = strToFloats (head xs); r = (read :: String -> Float) (xs !! 1); c = strToFloats (xs !! 2) in isInCircle p c r
        boolToStr True = "true"
